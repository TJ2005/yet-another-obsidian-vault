...
- Performs well for **linear datasets**
- **Easy to interpret, train** and retrain quickly
- **Models are likely to over-fit**, but this can be mitigated using regularization and cross-validation

### Cons

- Assumes **linearity between the dependent and independent variables**
- **Sensitive to outliers**, so preprocessing is essential
- Assumes no **multicollinearity between independent attributes**, requiring dimensionality reduction if violated
